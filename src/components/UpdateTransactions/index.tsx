...
const UpdateTransactions = () => {
  ...
  const { shouldShowUpdatePopup, onCancel, apiCall } =
  ...
  const [updateTransactionModel] = useState(() => {
    const { transactionName, type, category, amount, date, id } =
  ...
  useEffect(() => {
    const valuesAreNotUndefined =
      transaction.updateList.transactionName &&
      transaction.updateList.type &&
      transaction.updateList.category &&
      transaction.updateList.amount &&
      transaction.updateList.date;
  ...
  const onUpdateTransaction = async (
    ...
    const parsedDate = parse(
      updateTransactionModel.date.slice(0, 10),
      "yyyy-MM-dd",
      new Date(),
    );
    const formatDate = format(parsedDate, "yyyy-MM-dd'T'HH:mm:ssxxx");
    const data = {
      id: transaction.updateList.id,
      date: formatDate,
      type: updateTransactionModel.type,
      name: updateTransactionModel.transactionName,
      category: updateTransactionModel.category,
      amount: updateTransactionModel.amount,
    };
    const url =
    ...
    transactionData = {
      transactionName: transactionData.transaction_name,
    ...
    const list = new TransactionModel(
      transactionData.transactionNme,
    ...
    shouldShowUpdatePopup && (
      <div className="add-transaction-card">
        <form
          ...
          className="add-transaction-form-card"
        >
              ...
              <div className="add-transaction-data-card">
                <div className="add-transaction-input-card">
                  ...
                  <input
                    ...
                    onBlur={onBlurName}
                    onChange={(event) =>
                      updateTransactionModel.setName(event.target.value)
                    }
                ...
                <div className="add-transaction-input-card">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateTransactionModel.setType(event.target.value)
                    }
                    className="add-transaction-input"
                ...
                <div className="add-transaction-input-card">
                  <label
                    className="add-transaction-label"
                    htmlFor="updateTransacCat"
                  >
                    Transaction Category
                  </label>
                  <select
                    onChange={(event) =>
                      updateTransactionModel.setCategory(event.target.value)
                    }
                ...
                <div className="add-transaction-input-card">
                  ...
                  <input
                    onBlur={onBlurAmount}
                    onChange={(event) =>
                      updateTransactionModel.setAmount(event.target.value)
                    }
                ...
                <div className="add-transaction-input-card">
                  ...
                  <input
                    onBlur={onBlurDate}
                    onChange={(event) =>
                      updateTransactionModel.setDate(event.target.value)
                    }
                    value={
                      updateTransactionModel.date !== undefined
                        ? format(
                            parseISO(updateTransactionModel.date),
                            "yyyy-MM-dd",
                          )
                        : format(
                            parseISO(transaction.updateList.date),
                            "yyyy-MM-dd",
                          )
                    }
                    className="add-transaction-input"
